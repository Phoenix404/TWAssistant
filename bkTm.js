...
var arrivalTime = new Date(document.getElementById("back_time").value.split(":").slice(0,3).join(":"));
	var coord1 = document.getElementById('snipe_coord').value;
...
try{
	if(document.getElementById('combined_table'))
	{
		...
		var ver=window.game_data.version.match(/[\d|\.]+/g);
		...
		else if(window.$("#snipe_output").length <= 0){
			var msPerSec=1000;
			var secsPerMin=60;
			var minsPerHour=60;
			var hrsPerDay=24;
